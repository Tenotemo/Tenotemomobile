...
async function registerShare(c){const r=await rpc('tenotemo_register_campaign_share',{p_campaign_id:c.id});note(`Share recorded. ${money(r.provisional_cents||200)} is now provisional. Upload proof for up to R2 to move to Earned.`);await refresh();return r}
...
async function shareMedia(c){
 ...
 if(navigator.share&&navigator.canShare?.({files:[file]})){
  try{await navigator.share({files:[file],text:message,title:c.title});await registerShare(c);note('Media share recorded. R2 provisional is reserved; submit your platform proof to move the verified amount to Earned.');return}catch(e){if(e.name==='AbortError')return;throw e}
 }
...
async function refresh(){if(!tUser)return note('Sign in to use Tenotemo Earn.');note('Loading your private dashboard…');try{
 ...
 const proof=document.createElement('div');proof.style.cssText='margin-top:10px;padding:10px;border:1px solid #cbd5e1;border-radius:10px';proof.append(text('strong','Submit proof after sharing · R2 is provisional automatically; verification moves R1 per qualifying platform to Earned (max R2).'));
...
$('earnClaim').onclick=async()=>{const b=$('earnClaim');b.disabled=true;try{const rows=await rpc('tenotemo_claim_daily_challenges_today');const r=rows[0];note(`Claimed ${r.completion_credits} today's Daily Challenge Memory Credits.`);await refresh()}catch(e){note(e.message)}finally{b.disabled=false}};
$('earnClaimTop50').onclick=async()=>{const b=$('earnClaimTop50');b.disabled=true;try{const d=$('earnClaimDate').value;const rows=await rpc('tenotemo_claim_top50_day',{p_date:d});const r=rows[0];note(`Claimed ${r.top50_credits} Top 50 Memory Credits for ${d}.`);await refresh()}catch(e){note(e.message)}finally{b.disabled=false}};
